...
class Exporter extends \lithium\core\StaticObject {
	...
	public static function cast($data, $schema, $connection, array $options = array ()) {
		$defaults = array (
			'handlers' => array (),
			'model' => null,
			'arrays' => true,
		...
		foreach ($data as $key => $value) {
			...
			$data[$key] = static::_cast($value, $field, $connection, compact('pathKey') + $options);
		}
		return $data;
	}

	protected static function _cast($value, $def, $connection, $options) {
		if (is_object($value)) {
			return $value;
		}
		...
		elseif (is_array($value)) {
			$arrayType = !false && (array_keys($value) === range(0, count($value) - 1));
			$opts = $arrayType ? array ('class' => 'array') + $options : $options;
		}

		unset($opts['handlers'], $opts['first']);
		return $connection->item($options['model'], $value, compact('pathKey') + $opts);
	}
	...
	protected static function _update($export) {
		...
		array_map(function($key) use (&$left) {
				unset($left[$key]);
			}, array_keys($right));
